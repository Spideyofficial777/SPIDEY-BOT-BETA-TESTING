...
from database.multi_db_manager import multi_db_manager, get_user_database
...
# Since we're using file storage databases now, we'll use the main database for group data
group_db = None
class Database:
    default = SETTINGS.copy()
    def __init__(self):
        # Use multi-database connections for better data organization
        self.col = user_db.users if user_db is not None else mydb.users
        self.grp = group_db.groups if group_db is not None else mydb.groups
        self.misc = user_db.misc if user_db is not None else mydb.misc
        self.verify_id = user_db.verify_id if user_db is not None else mydb.verify_id
        self.users = user_db.uersz if user_db is not None else mydb.uersz
        self.req = user_db.requests if user_db is not None else mydb.requests
        self.mGrp = group_db.mGrp if group_db is not None else mydb.mGrp
        self.pmMode = user_db.pmMode if user_db is not None else mydb.pmMode
        self.spidey_ads_link = user_db.spidey_ads_link if user_db is not None else mydb.spidey_ads_link
        self.grp_and_ids = fsubs.grp_and_ids  # Keep this on legacy connection
        self.movies_update_channel = group_db.movies_update_channel if group_db is not None else mydb.movies_update_channel
        self.botcol = user_db.botcol if user_db is not None else mydb.botcol
        
        # Fallback to legacy connections if multi-db fails
        if user_db is None:
            self.col = mydb.users
            self.misc = mydb.misc
