...
from database.multi_db_manager import get_file_storage_1_database

class Database:
    def __init__(self, uri, db_name):
        # Try to use multi-database system first
        config_db = get_file_storage_1_database()
        if config_db is not None:
            self.db = config_db
            self.client = None  # Managed by multi_db_manager
        else:
