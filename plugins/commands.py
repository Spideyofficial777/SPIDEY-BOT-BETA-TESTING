...
from utils import * # formate_file_name,  get_settings, save_group_settings, is_req_subscribed, get_size, get_shortlink, is_check_admin, get_status, temp, get_readable_time, save_default_settings, is_subscribed
import re
import base64
from info import *
import traceback
...
async def start(client:Client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('• ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ •', url="https://t.me/+CLjMaOl5SuA0ZTg1")
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if not await db.is_user_exist(message.from_user.id):
        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if MULTI_FSUB and await is_req_subscribed(client, message):
                        buttons = [[
        ...
        InlineKeyboardButton(text="🖥", callback_data="main"),
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAEBN59oAAFloQpI_OBOJwABcp8jut-Q6D6AAAKrBAACvu-4V0dQs1WLoficNgQ") 
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/Spidey_support').json()
